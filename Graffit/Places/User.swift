...
class User {
  ...
  func createPost(postText: String, longitude: Double, latitude: Double, token: String) -> Void {
    ...
    do {
      ...
      print("json string = \(jsonString)")
      print("json data = \(jsonData)")
      let url = NSURL(string: "http://ec2-54-242-147-65.compute-1.amazonaws.com:3333/user/post")!
      let request = NSMutableURLRequest(url: url as URL)
      request.httpMethod = "POST"
      request.addValue("application/json", forHTTPHeaderField: "Content-Type")
      print("Before adding jsonData")
      ...
      let urlString = URL(string: "http://ec2-54-242-147-65.compute-1.amazonaws.com:3333/user/post")
      let session = URLSession.shared
      let request2 = NSMutableURLRequest(url: urlString!)
      request2.httpMethod = "POST"
      request.cachePolicy = NSURLRequest.CachePolicy.reloadIgnoringCacheData
